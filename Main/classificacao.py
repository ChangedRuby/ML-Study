...
def classificar_mqo(X_treino, y_treino, X_teste):
    Y_treino, rotulos = criar_Y(y_treino)

    X_treino_mqo = np.column_stack((
        np.ones(len(X_treino)),
        X_treino
    ))

    X_teste_mqo = np.column_stack((
        np.ones(len(X_teste)),
        X_teste
    ))

    W = np.linalg.pinv(X_treino_mqo) @ Y_treino

    pontuacoes = X_teste_mqo @ W
    indices_classes = np.argmax(pontuacoes, axis=1)

    return rotulos[indices_classes]


def classificar_mqo_regularizado(X_treino, y_treino, X_teste, lamb):
    Y_treino, rotulos = criar_Y(y_treino)
    ...
    I = np.eye(X_treino_mqo.shape[1])

    W = np.linalg.pinv(
        X_treino_mqo.T @ X_treino_mqo + lamb * I
    ) @ X_treino_mqo.T @ Y_treino
...
def classificar_mqo_polinomial(X_treino, y_treino, X_teste, q):
    Y_treino, rotulos = criar_Y(y_treino)

    X_treino_normalizado = X_treino / 4095.0
    X_teste_normalizado = X_teste / 4095.0

    X_treino_poly = criar_polinomio(
        X_treino_normalizado,
        q
    )

    X_teste_poly = criar_polinomio(
        X_teste_normalizado,
        q
    )

    W = np.linalg.pinv(X_treino_poly) @ Y_treino

    pontuacoes = X_teste_poly @ W
    indices_classes = np.argmax(pontuacoes, axis=1)
